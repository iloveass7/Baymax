...
export const getallUser = async (req, res) => {
    try {
        ...
        res.status(404).json({ success: false, message: "Not found any users." });
    }
};
...
export const getUserProfile = async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ success: true, message: 'Profile info is laoding', data: { ...rest } })
...
export const getMyAppointments = async (req, res) => {
    try {
        ...
        res.status(200).json({ success: true, message: 'Appointments loaded', data: doctors })
    } catch (err) {
